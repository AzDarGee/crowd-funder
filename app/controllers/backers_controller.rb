class BackersController < ApplicationController
  ...
  def create
    ...
    if @backer.save
      redirect_to root_path, flash: { notice: '#{backer.user_name} signed up. Start backing some projects!' }
    else
  ...
  def backer_params
    params.require(:backer).permit(:user_name,:email,:password,:password_digest)
  end
end
